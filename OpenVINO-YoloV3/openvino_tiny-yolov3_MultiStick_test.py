...
def aggregateThread(api_results):
    def _aggregate():
...
if __name__ == '__main__':
    ...
    try:
        ...
        # Start streaming
        p = mp.Process(target=camThread, args=(LABELS, results, frameBuffers, camera_width, camera_height, vidfps), daemon=True)
        p.start()
        processes.append(p)

        # Start combining outputs
        p = mp.Process(target=aggregateThread, args=(api_results,), daemon=True)
        p.start()
        processes.append(p)
